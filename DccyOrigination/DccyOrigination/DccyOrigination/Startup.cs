    ...
    public class Startup
    {
        ...
        public void ConfigureServices (IServiceCollection services)
        {
            ...
            services.Configure<CookiePolicyOptions>(options =>
            {
                ...
                options.CheckConsentNeeded = context => false;             //当为true时session不可用
            ...
            //注入session必指定过期时间，httPonly=true 防止oxx攻击 设置在浏览器不能通过js获得该cookie的值
            services.AddDistributedMemoryCache();//session保存，
            int dieouttime = Configuration.GetSection("AppSetting:SessionState").GetValue<int>("IdleTimeout") <= 0 ? 30 : Configuration.GetSection("AppSetting:SessionState").GetValue<int>("IdleTimeout");
            services.AddSession(options => { options.IdleTimeout = TimeSpan.FromSeconds(dieouttime); options.Cookie.HttpOnly = true; }); //设置session的过期时间
